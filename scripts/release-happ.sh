...
#   1. `workdir/tables.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/tables.happ"
else
  happ="workdir/tables.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical tables.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download tables.happ from $HAPP_TAG."
